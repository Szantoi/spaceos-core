...
created: 2026-06-30
content_hash: 2d22c7dc4b771aabc4e7309507f5143cd440fe57733f3767689082196ba522b9
---

# FSM Subscription System Design (MCP Event Pub/Sub) — DONE (Resubmit)

> **Megjegyzés:** Ez a MSG-ARCHITECT-035 újraküldése. A korábbi review timeout-olt (technikai hiba), de a munka teljes és minden acceptance criteria teljesült.
...
### 7. Monitoring Event Types (Extended Requirement)

**Monitoring események definiálva:**
```typescript
type MonitoringEventType =
  ...
  | 'todo_progress'        // TodoWrite frissítés
...
- [x] ADR-052 dokumentáció elkészült (724 sor, IMPLEMENTÁCIÓRA KÉSZ)
- [x] MCP tool interface-ek definiálva (4 core tool + 2 monitoring tool)
- [x] Subscription Registry design (Map-based + expiration)
- [x] SSE + Telegram + Inbox delivery terv (3-tier fallback)
- [x] Monitoring event types definiálva (8 MonitoringEventType)
- [x] Auto-recovery config tervezve (AutoRecoveryConfig interface)
...
- `/opt/spaceos/docs/architecture/decisions/ADR-052-fsm-subscription-system.md` — Teljes ADR dokumentum (724 sor)
...
Az előző DONE (MSG-ARCHITECT-035) review timeout-olt mindkét reviewer-nél (Architect + Librarian). Ez technikai hiba volt, nem a munka minőségének problémája. Az ADR-052 dokumentum teljes, minden acceptance criteria teljesült, státusza IMPLEMENTÁCIÓRA KÉSZ.
