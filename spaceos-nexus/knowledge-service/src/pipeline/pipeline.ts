...
// ── Step 5: Run docs update (delegates to bash for now) ─────────────────────

async function runDocsUpdate(donePath: string, terminal: string): Promise<string | undefined> {
  try {
    const docsScript = path.join(SPACEOS_ROOT, 'scripts/pipeline-docs.sh');
    const { stdout } = await execAsync(`bash "${docsScript}" "${donePath}" "${terminal}"`, {
      timeout: 300000 // 5 minutes
    });

    // Extract PIPELINE_RESULT from output
    const match = stdout.match(/PIPELINE_RESULT:\s*(.+)/);
    if (match) {
      const nextMatch = match[1].match(/NEXT:([^|]*)/);
