...
| `project_jwkshealthcheck_public_visibility.md` | project | `JwksHealthCheck` must remain `public` — `Program.cs` uses it as a generic type arg; making it internal causes CS0122. Unfixable without factory-lambda registration or adding Api to InternalsVisibleTo. |
