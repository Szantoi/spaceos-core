---
id: workflow-backend_developer_go
title: "Backend Developer Initialization Workflow"
description: "Task execution workflow for the Backend Developer: plan loading, skill loading, implementation, verification, and QA handoff."
type: workflow
scope: global
...
**Objective**: Implement the features described in the **[[ TASK_ID ]]** plan following Clean Architecture principles.

---

### Cognitive Setup

1. **N-shot Pattern** — Review existing code in the repository for style and architectural patterns. Follow them.
2. **Fact Summary Pattern** — At the end of the task, concisely summarize all changes made.
...
3. **Response Sending**:
    - [ ] Create a response message to the Orchestrator with:
      ...
      - Body: List of changed files, path to the implementation summary, and the next step (QA testing if required)
...
- **P11**: `qa_tester_qa_handoff.message.md`
- **When**: Backend implementation complete; QA testing required
- **Parameters**: `{TASK_ID}`, `{EPIC_ID}`, `{changed_files}`, `{edge_cases}`
