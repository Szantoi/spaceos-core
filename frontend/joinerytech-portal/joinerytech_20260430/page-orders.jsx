...
function OrderRow({ o, t, isOpen, onToggle, onNav }) {
      ...
      {isOpen && (
        <div className="px-5 pb-5 pt-2 bg-stone-50/40 border-t border-stone-100">
          ...
          <div className="flex items-center gap-2">
            ...
            {liveStatus === "ready" && (
              <PrimaryBtn icon="external" onClick={() => window.releaseToWorkflow(o.id)}>Kiadás gyártásba</PrimaryBtn>
            )}
            {liveStatus === "released" && (
        ...
        <div className="flex items-center gap-1 bg-white border border-stone-200 rounded-lg p-0.5">
          {filters.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-2.5 h-7 rounded-md text-[12px] inline-flex items-center gap-1.5 ${filter === f ? "bg-stone-900 text-white" : "text-stone-600 hover:bg-stone-100"}`}>
