...
tools:
  ...
  - mcp__semgrep__deprecation_notice
...
Before reviewing any code, read the `CLAUDE.md` file in the root and in the relevant `src/` subdirectories. These files define the architectural constraints that inform the threat model:
- `/CLAUDE.md` — Golden Rules (all HTTP via `/bff`, no direct Kernel calls, Zustand owns only auth state)
- `src/api/CLAUDE.md` — JWT interceptor, 401/429 handling, service layer boundaries
