...
Before reviewing any architectural decision, read the `CLAUDE.md` file in the root and in each affected `src/` subdirectory. These files define the established architectural contracts for this Layer 4 frontend:
- `/CLAUDE.md` — Layer role, Golden Rules, approved packages
- `src/api/CLAUDE.md` — BFF proxy pattern, aggregate-per-file service structure
- `src/hooks/CLAUDE.md` — TanStack Query as the server state layer
- `src/store/CLAUDE.md` — Zustand scope (auth only)
