/* AUTO-GENERATED from page-floorplan.jsx — NE SZERKESZD, a forrás a .jsx! */
(function(){
// ──────────────────────────────────────────────────────────────────────────
...
function fp4dForComp(comp, s) {
  ...
  if (["draft", "sent", "rejected", "expired"].includes(q.status)) return {
    stage: "ajanlat",
    frac: 0
  };
...
function FloorplanStudio() {
  ...
  useEffectFp(() => {
    if (concept && !fp) window.sim.ensureFloorplan(concept.id);
  }, [concept && concept.id, !!fp]);
  ...
  if (!fp) return /*#__PURE__*/React.createElement("div", {
    className: "px-4 md:px-7 py-6 text-stone-400 text-[12px]"
  }, "T\xE9r el\u0151k\xE9sz\xEDt\xE9se\u2026");
  ...
  fp.rooms.forEach(r => Object.entries(r.walls || {}).forEach(([side, compoId]) => {
    ...
    items.forEach((p, i) => projected.push({
      ...p,
      roomId: r.id,
      roomName: r.name,
      stage: fp4dStageForIndex(res, i, items.length),
      wallIdx: i + 1,
  ...
  const allRects = [...fp.rooms, ...(fp.furn || []).map(f => ({
    x: f.x,
    y: f.y,
    w: f.rot ? f.d : f.w,
    h: f.rot ? f.w : f.d
  ...
  const onCanvasClick = e => {
    const p = toMm(e);
    if (mode === "room") {
      const id = window.sim.addFpRoom(concept.id, {
        x: p.x,
        y: p.y
      });
      setSel({
      ...
      const id = window.sim.addFpZone(concept.id, {
        x: p.x,
        y: p.y,
        roomId: host ? host.id : null
      });
      setSel({
        type: "zone",
        id
      ...
      const id = window.sim.addFpFurn(concept.id, {
        label: palettePick.label,
        tplId: palettePick.tplId || null,
        w: palettePick.w,
        d: palettePick.d,
        x: p.x,
        y: p.y
  ...
  const onMove = e => {
    ...
    if (d.resize) {
      d.live.w = Math.max(400, p.x - d.live.x);
      const hk = d.kind === "furn" ? "d" : "h";
      d.live[hk] = Math.max(400, p.y - d.live.y);
    } else {
      d.live.x = p.x - d.ox;
      d.live.y = p.y - d.oy;
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "T\xE9rrendez\xE9s"), /*#__PURE__*/React.createElement("div", {
    className: "text-[11.5px] text-stone-500 max-w-xl"
  }, "Helyis\xE9gek, z\xF3n\xE1k, b\xFAtor-kont\xFArok \u2014 a kont\xFAr a m\u0171szaki tervez\xE9s parametrikus v\xE1z\xE1nak alacsony felbont\xE1s\xFA felold\xE1sa. Minden szint csak annyit t\xF6lt be, amennyi kell.")), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-1.5 flex-wrap"
  }, concepts.map(c => /*#__PURE__*/React.createElement("button", {
    key: c.id,
    onClick: () => {
      setCid(c.id);
      setSel(null);
    },
    className: `h-8 px-2.5 rounded-lg border text-[11.5px] font-medium ${c.id === concept.id ? "border-rose-300 bg-rose-50 text-rose-700" : "border-stone-200 bg-white text-stone-600 hover:border-stone-300"}`
  }, c.name)))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-1.5 flex-wrap"
  }, [["select", "Kijelölés"], ["room", "+ Helyiség"], ["zone", "+ Zóna"], ["furn", "+ Bútor-kontúr"]].map(([k, lbl]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => {
      setMode(k);
      if (k !== "furn") setPalettePick(null);
    },
    className: `h-9 px-3 rounded-lg border text-[12px] font-medium ${mode === k ? "border-rose-400 bg-rose-50 text-rose-700 ring-1 ring-rose-200" : "border-stone-200 bg-white text-stone-600 hover:border-stone-300"}`
  }, lbl)), /*#__PURE__*/React.createElement("span", {
    className: "flex-1"
  }), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center rounded-lg border border-stone-200 bg-white overflow-hidden"
  }, [["terv", "Terv"], ["4d", "4D készültség"], ["szerelo", "Szerelő"]].map(([k, lbl]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => {
      setLayer(k);
      setScanHit(null);
    },
    className: `h-9 px-3 text-[12px] font-medium ${layer === k ? "bg-stone-900 text-white" : "text-stone-600 hover:bg-stone-50"}`
  }, lbl)))), /*#__PURE__*/React.createElement("div", {
    className: "text-[10.5px] text-stone-400 -mt-2"
  }, layer === "4d" ? "4D: a szín az elem helye az IDŐBEN — az ajánlat → rendelés → gyártási feladat láncból számítva, nem tárolt." : layer === "szerelo" ? "Szerelő nézet: olvasd be (írd be) az elem kódját a jobb oldali panelen — a tér megmutatja, melyik szekrénybe megy." : mode === "room" ? "Kattints a vászonra — 3×3 m helyiség kerül oda." : mode === "zone" ? "Kattints egy helyiségbe — zóna kerül oda." : mode === "furn" ? palettePick ? `Kattints a vászonra: ${palettePick.label}` : "Válassz modellt a jobb oldali palettáról." : "Kattints / húzd az alakzatokat. Sarok-fogantyú = méretezés."), /*#__PURE__*/React.createElement("div", {
    className: "grid lg:grid-cols-3 gap-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lg:col-span-2 rounded-2xl border border-stone-200 bg-white overflow-hidden"
    ...
    return /*#__PURE__*/React.createElement("g", {
      ...
      width: r.w,
      height: r.h,
      fill: isSel ? "rgba(244,63,94,.05)" : "#fafaf9",
      stroke: isSel ? "#f43f5e" : "#57534e",
      strokeWidth: isSel ? 50 : 70,
      style: {
        cursor: "move"
      },
      onPointerDown: e => mode === "select" && startDrag(e, "room", r0),
      ...
      fontWeight: "600",
      fill: "#44403c"
    }, r.name), /*#__PURE__*/React.createElement("text", {
      x: r.x + 140,
      y: r.y + 560,
      fontSize: "170",
      fill: "#a8a29e",
      fontFamily: "monospace"
    }, (r.w * r.h / 1e6).toFixed(1), " m\xB2"), Object.entries(r0.walls || {}).filter(([, v]) => v).map(([side]) => {
      const t = 90;
      const seg = side === "N" ? {
        x: r.x,
        y: r.y - t,
      ...
      onPointerDown: e => startDrag(e, "room", r0, true)
    }));
  }), (fp.zones || []).map(z0 => {
    ...
    return /*#__PURE__*/React.createElement("g", {
      ...
      fill: tone.fill,
      stroke: tone.stroke,
      strokeWidth: isSel ? 40 : 20,
      strokeDasharray: "140 90",
      rx: "60",
      style: {
        cursor: "move"
      },
      onPointerDown: e => mode === "select" && startDrag(e, "zone", z0),
      onClick: e => e.stopPropagation()
    }), /*#__PURE__*/React.createElement("text", {
      x: z.x + 110,
      y: z.y + 260,
      ...
      fontWeight: "600",
      fill: tone.text
    }, z0.name), isSel && /*#__PURE__*/React.createElement("rect", {
      x: z.x + z.w - 180,
      y: z.y + z.h - 180,
      width: "360",
      height: "360",
      ...
      onPointerDown: e => startDrag(e, "zone", z0, true)
    }));
  }), projected.map(p => {
    ...
    return /*#__PURE__*/React.createElement("g", {
      ...
      fill: fill,
      stroke: stroke,
      strokeWidth: isSel || isHit ? 48 : 24,
      rx: "40"
    }), isHit && /*#__PURE__*/React.createElement("rect", {
      x: p.x - 110,
      y: p.y - 110,
      width: p.w + 220,
      height: p.h + 220,
      ...
      fontSize: "150",
      textAnchor: "middle",
      fill: layer === "4d" ? "#44403c" : "#9f1239"
    }, p.it.tplName ? p.it.tplName.slice(0, 14) : ""));
  }), (fp.furn || []).map(f0 => {
    const f = liveOf("furn", f0);
    const w = f0.rot ? f.d : f.w,
      h = f0.rot ? f.w : f.d;
    ...
    return /*#__PURE__*/React.createElement("g", {
      ...
      width: w,
      height: h,
      fill: isSel ? "rgba(120,113,108,.16)" : "rgba(120,113,108,.08)",
      stroke: "#78716c",
      strokeWidth: isSel ? 40 : 24,
      strokeDasharray: f0.tplId ? "" : "120 80",
      rx: "50",
      style: {
        cursor: "move"
      },
      onPointerDown: e => mode === "select" && startDrag(e, "furn", f0),
      onClick: e => e.stopPropagation()
    }), /*#__PURE__*/React.createElement("text", {
      x: f.x + w / 2,
      y: f.y + h / 2 + 55,
      ...
      textAnchor: "middle",
      fill: "#57534e"
    }, f0.label.slice(0, 16)), f0.tplId && /*#__PURE__*/React.createElement("circle", {
      cx: f.x + 130,
      cy: f.y + 130,
      r: "70",
      fill: "#f59e0b"
    ...
    className: "px-4 py-2 border-t border-stone-100 bg-stone-50/60 flex items-center gap-3 flex-wrap text-[10.5px] text-stone-400"
  }, layer === "4d" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    className: "font-medium text-stone-500"
  }, "4D k\xE9sz\xFClts\xE9g:"), Object.values(FP4D_STAGES).map(st => /*#__PURE__*/React.createElement("span", {
    key: st.label,
  ...
  }), st.label))) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-3 h-3 rounded-sm border-2 border-stone-500 bg-stone-50"
  }), "helyis\xE9g"), /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-3 h-3 rounded-sm border border-dashed border-emerald-500 bg-emerald-50"
  }), "z\xF3na"), /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-3 h-3 rounded-sm border border-stone-500 bg-stone-100"
  }), "kont\xFAr ", /*#__PURE__*/React.createElement("span", {
    className: "w-1.5 h-1.5 rounded-full bg-amber-500 inline-block"
  }), " = skeleton"), /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-3 h-3 rounded-sm border border-rose-500 bg-rose-50"
  }), "b\xFAtorsor-kivet\xEDt\xE9s"), /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-3 h-2 rounded-sm bg-rose-500/75"
  }), "linkelt faln\xE9zet")))), /*#__PURE__*/React.createElement("div", {
    className: "space-y-4"
  }, layer === "szerelo" && window.FpScanPanel ? /*#__PURE__*/React.createElement(window.FpScanPanel, {
    projected: projected,
    fp: fp,
    scanHit: scanHit,
    setScanHit: setScanHit,
    compos: compos
  }) : window.FpInspector ? /*#__PURE__*/React.createElement(window.FpInspector, {
    concept: concept,
    fp: fp,
    sel: sel,
    setSel: setSel,
