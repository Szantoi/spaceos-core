...
})[k] || { l: k, icon: "user", tint: "bg-stone-100 text-stone-600" };

const SEG_META = {
  step:     { l: "Lépés",      icon: "box",      tint: "text-stone-600" },
  branch:   { l: "Elágazás",   icon: "workflow", tint: "text-violet-600" },
  parallel: { l: "Párhuzamos", icon: "layers",   tint: "text-sky-600" },
  loop:     { l: "Ciklus",     icon: "external", tint: "text-amber-600" },
};

let _segN = 0;
const segId = (p) => (p || "seg") + "-" + Date.now().toString(36) + (_segN++).toString(36);

const newStep = (over = {}) => ({ id: segId("s"), kind: "step", name: "Új lépés", phase: "Gyártás", actor: "manufacturer", external: false, partnerType: "supplier", sla: 8, subtasks: [], ...over });
const newBranch = () => ({ id: segId("b"), kind: "branch", prompt: "Döntés?", paths: [
  { id: segId("p"), label: "Igen", cond: "", flow: [newStep({ name: "Ág A" })] },
  { id: segId("p"), label: "Nem", cond: "", flow: [newStep({ name: "Ág B" })] },
] });
const newParallel = () => ({ id: segId("par"), kind: "parallel", lanes: [
  { id: segId("l"), label: "Sáv 1", flow: [newStep({ name: "Párhuzamos A" })] },
  { id: segId("l"), label: "Sáv 2", flow: [newStep({ name: "Párhuzamos B" })] },
] });
const newLoop = (targetId) => ({ id: segId("lp"), kind: "loop", label: "Visszacsatolás", cond: "", targetId: targetId || null });
...
function mapFlow(flow, fn) {
  ...
  (flow || []).forEach((seg) => {
    let s = fn(seg);
    if (s === null) return;
    if (s.kind === "branch") s = { ...s, paths: s.paths.map((p) => ({ ...p, flow: mapFlow(p.flow, fn) })) };
    else if (s.kind === "parallel") s = { ...s, lanes: s.lanes.map((l) => ({ ...l, flow: mapFlow(l.flow, fn) })) };
    out.push(s);
  });
  return out;
}
...
function insertSeg(flow, container, seg, afterId) {
  const addInto = (list) => {
    ...
    const n = [...list]; n.splice(i + 1, 0, seg); return n;
  };
  if (container.scope === "root") return addInto(flow);
  return mapFlow(flow, (s) => {
    if (s.kind === "branch") return { ...s, paths: s.paths.map((p) => p.id === container.pathId ? { ...p, flow: addInto(p.flow) } : p) };
    if (s.kind === "parallel") return { ...s, lanes: s.lanes.map((l) => l.id === container.laneId ? { ...l, flow: addInto(l.flow) } : l) };
    return s;
  });
}
...
function moveSeg(flow, id, dir) {
  ...
  let touched = false;
  const recur = (list) => {
    if (list.some((x) => x.id === id)) { touched = true; return swap(list); }
    return list.map((s) => {
      if (s.kind === "branch") return { ...s, paths: s.paths.map((p) => ({ ...p, flow: recur(p.flow) })) };
      if (s.kind === "parallel") return { ...s, lanes: s.lanes.map((l) => ({ ...l, flow: recur(l.flow) })) };
      return s;
    });
  };
  return recur(flow);
...
function addPath(flow, branchId) {
  return updateSeg(flow, branchId, (b) => ({ ...b, paths: [...b.paths, { id: segId("p"), label: "Új ág", cond: "", flow: [newStep({ name: "Új lépés" })] }] }));
}
function removePath(flow, branchId, pathId) {
  return updateSeg(flow, branchId, (b) => ({ ...b, paths: b.paths.length > 1 ? b.paths.filter((p) => p.id !== pathId) : b.paths }));
}
function addLane(flow, parId) {
  return updateSeg(flow, parId, (pp) => ({ ...pp, lanes: [...pp.lanes, { id: segId("l"), label: "Új sáv", flow: [newStep({ name: "Új lépés" })] }] }));
}
function removeLane(flow, parId, laneId) {
  return updateSeg(flow, parId, (pp) => ({ ...pp, lanes: pp.lanes.length > 1 ? pp.lanes.filter((l) => l.id !== laneId) : pp.lanes }));
}
// all step segments (for loop targets), flattened with order
function allSteps(flow) {
  const out = [];
  const recur = (list) => (list || []).forEach((s) => {
    if (s.kind === "step") out.push(s);
    else if (s.kind === "branch") s.paths.forEach((p) => recur(p.flow));
    else if (s.kind === "parallel") s.lanes.forEach((l) => recur(l.flow));
  });
  recur(flow);
  return out;
}
...
Object.assign(window, {
  PROC_PALETTE, PROC_ACTORS, procActor, SEG_META, segId,
  newStep, newBranch, newParallel, newLoop,
  mapFlow, updateSeg, removeSeg, insertSeg, moveSeg, addPath, removePath, addLane, removeLane, allSteps,
});
