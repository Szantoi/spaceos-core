s# woodwork_domain.md — Asztalos / faipari SZAKMAI tudás
...
- `docs/faipari_gyartasszervezes_rag.md` — Soponyai Éva: Faipari gyártásszervezés teljes RAG export (11 fejezet, 5975 sor). A 22-23. szakaszok forrása.
- `docs/faipari_muszaki_dokumentacio_rag.md` — Neiser Ákos: Faipari műszaki dokumentáció teljes RAG export (4 fejezet, 2489 sor, Szega Books 2023). A 22. szakasz forrása.

---

## 22. Vizsgatevékenységek dokumentációs követelményei (2024)

> Forrás: Neiser Ákos: *Faipari műszaki dokumentáció* (Szega Books, 2023), 1.1. táblázat.

A szakképzési kerettanterv 2024-es változata **két vizsgarész** dokumentációját különbözteti meg:

| Vizsgarész | Tevékenység | Kötelező dokumentáció |
|---|---|---|
| **I. Portfólió készítése** | Tervezés + dokumentálás | • Műszaki rajz<br>• Szabásjegyzék<br>• Anyagnorma<br>• Munkalap / műveletterv<br>• Árkalkuláció |
| **II. Vizsgaremek készítése** | Gyártás + összeszerelés | • Alkatrészjegyzék<br>• Szerelvényjegyzék<br>• Segédanyagnorma<br>• Vonalas folyamatábra<br>• Árkalkuláció |

### Különbség a portfólió és a vizsgaremek között

- **Portfólió** = tervezési dokumentumcsomag (műszaki rajz + szabás + anyagnorma + műveletterv + ár).
- **Vizsgaremek** = gyártási dokumentumcsomag (alkatrész + szerelvény + folyamat + ár).

**SpaceOS vonatkozás:**
- A **portfólió generálás** = `Quote` → `TechnicalDrawing` + `CuttingPlan` + `MaterialNorm` + `WorkPlan` + `PriceCalculation` flow.
- A **vizsgaremek generálás** = `ProductionOrder` → `BOM` + `Assembly` + `ProcessFlow` + `FinalPrice` flow.

**Státusz:**
- ⏳ Még nincs külön endpoint a portfólió vs. vizsgaremek dokumentum-csomagra.
- 💡 **Jövőbeli feature:** `GET /api/joinery/portfolio/{quoteId}` és `GET /api/joinery/exam-artifact/{orderId}`.

---

## 23. Gyártási rendszerek mélyebb szemlélete

> Forrás: Soponyai Éva: *Faipari gyártásszervezés* (Skandi-Wald, 4. kiadás), 1-2. fejezetek.

### 23.1. Rendszerszervezési elvek

A gyártási rendszer **sorozatos (soros) vagy párhuzamos** felépítésű lehet:

| Típus | Működés | Példa |
|---|---|---|
| **Sorozatos kapcsolás** | A műveletek egymás után, sorban követik egymást. Egy művelet kimenetje a következő bemenetévé válik. | CNC szabás → élzárás → fúrás → összeszerelés |
| **Párhuzamos kapcsolás** | Több művelet egyidejűleg fut különböző alkatrészeken vagy ugyanazon alkatrész különböző pontjain. | 3 asztalos 3 különböző fiókot szerel párhuzamosan |

### 23.2. Visszacsatolás és szabályozás

**Két fő szabályozási mód:**

1. **Nyitott rendszer (nincs visszacsatolás)** — A művelet kimenetét NEM ellenőrzik, NEM korrigálják automatikusan.
   - **Példa:** Kézi vágás sablonnal — nincs automatikus korrekció, csak az operátor ellenőrzi a végeredményt.

2. **Zárt rendszer (van visszacsatolás)** — A kimenet mérése alapján automatikusan korrigálódik a bemeneti paraméter.
   - **Példa:** CNC gép optikai mérőrendszerrel — ha a vágás pontatlan, automatikusan újrakalibrálja a fejállást.

**SpaceOS vonatkozás:**
- **Nyitott rendszer** = `WorkflowStep` nincs auto-korrekcióval → `FSM` státusz `Failed` esetén **manuális** beavatkozás szükséges.
- **Zárt rendszer** = jövőbeli **Quality Control loop** — `MeasurementEvent` → `WorkflowStep` auto-retry if tolerance exceeded.

**Státusz:**
- 🔨 Részben: FSM státuszok léteznek, de nincs automatikus visszacsatolás / korrekció.
- 💡 **Jövőbeli feature:** `QualityControlLoop` domain event → auto-retry vagy operator notifikáció.

### 23.3. Befoglaló méret (bounding box) számítási konvenciók

**Faipari konvenció:** A befoglaló méret számítása függ a termék típusától:

| Terméktípus | Befoglaló méret számítási elve |
|---|---|
| **Ajtó (door)** | Szélesség × Magasság × Vastagság |
| **Szekrény (cabinet)** | Szélesség × Mélység × Magasság |
| **Asztal (table)** | Szélesség × Mélység × Magasság (lábakkal együtt) |

**SpaceOS vonatkozás:**
- A `ParametricProduct` interfész `BoundingBox` mezője termék-típustól függ.
- A `Door` aggregate `Width × Height × Thickness` sorrendet használ.
- A `Cabinet` aggregate `Width × Depth × Height` sorrendet használ.

**Státusz:**
- ✅ Rendszerben modellezve: `BoundingBox` mező minden `ParametricProduct` interfészen.
- 🔨 Részben: Még nincs explicit validáció, hogy melyik termék melyik sorrendet használja.
