...
// ──────────────────────────────────────────────────────────────────────────
const { useEffect: useEffectMN } = React;

// screen-key → Icon name. Falls back to a sensible default per key pattern.
const SCREEN_ICON = {
  dash: "dashboard", analytics: "analytics",
  cutting: "cut", machining: "cpu", workflow: "workflow",
  orders: "orders", quotes: "briefcase", customers: "user",
  editor: "ruler", generate: "layers", catalog: "box",
  inventory: "inventory", procurement: "procurement", procurement2: "procurement", movements: "external",
  receiving: "download", withdrawals: "send", warehouse: "inventory", offcuts: "cut", stocktake: "layers",
  company: "briefcase", users: "user", facilities: "factory", machines: "cpu",
  partners: "user", integrations: "bolt", audit: "orders", roles: "settings",
  deliveries: "truck", pickups: "download", schedule: "calendar", terminal: "truck", resources: "user",
  projects: "briefcase", variance: "analytics",
  tickets: "shield", board: "workflow",
};
...
function MobileBottomNav({ world, accent, screen, onScreen, onMore, lang }) {
...
function NavTab({ icon, label, badge, active, activeBg, activeFg, onClick }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      >
        <span className={active ? activeFg : "text-stone-400"}>
          <Icon name={icon} size={20} />
        </span>
        {badge && (
          <span className="absolute -top-0.5 right-2.5 min-w-[7px] h-[7px] rounded-full bg-rose-500 ring-2 ring-white" />
        )}
      </span>
      <span
