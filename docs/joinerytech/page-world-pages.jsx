...
function ProductionDashboard({ onScreen }) {
  ...
  return (
    <div className="px-7 py-6 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card className="p-4">
          ...
          <div className="text-[10.5px] mt-1">
            ...
            {inMaint > 0 && <><span className="text-stone-400 mx-1">·</span><span className="text-amber-600">{inMaint} karbantartás</span></>}
          </div>
        </Card>
        <Card className="p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-0 overflow-hidden">
          <div className="px-5 py-3 border-b border-stone-100 flex items-center justify-between">
            ...
            <button onClick={() => window.navigateTo ? window.navigateTo("mfgprep", "nesting") : onScreen?.("machining")} className="text-[11.5px] text-teal-700 font-medium hover:underline">Anyagoptimalizálás →</button>
          </div>
          {machines.map(m => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { key: "prep",      label: "Gyártás-előkészítés", icon: "cpu",    desc: "Anyag, útvonal, kiadás a műhelynek", world: "mfgprep", screen: "dash" },
          ...
          <button key={s.key} onClick={() => s.world ? (window.navigateTo && window.navigateTo(s.world, s.screen)) : onScreen?.(s.key)}
...
const MOV_TONE = {
  ...
  "Selejt":   { bg: "bg-rose-50",    fg: "text-rose-700",    dot: "bg-rose-500" },
  "Zóna":     { bg: "bg-violet-50",  fg: "text-violet-700",  dot: "bg-violet-500" },
  "Mozgatás": { bg: "bg-indigo-50",  fg: "text-indigo-700",  dot: "bg-indigo-500" },
};
const MOV_TONE_DEFAULT = { bg: "bg-stone-100", fg: "text-stone-600", dot: "bg-stone-400" };
const movTone = (t) => MOV_TONE[t] || MOV_TONE_DEFAULT;

function MovementDetailSlideOver({ mov, onClose }) {
  if (!mov) return null;
  const tone = movTone(mov.type);
  const kv = [
    { label: "Dátum", value: mov.date, mono: true },
    { label: "Forrás / Cél", value: mov.src, mono: true, accent: true },
    { label: "Felelős", value: mov.who },
    { label: "Egység", value: mov.unit },
  ];
  return (
    <SlideOver open={true} onClose={onClose} title={mov.mat} subtitle={`Mozgás · ${mov.date}`} width={440}
      footer={<GhostBtn onClick={onClose}>Bezárás</GhostBtn>}>
      <div className="px-5 py-5 space-y-5">
        {/* hero: type + signed quantity */}
        <div className="flex items-center justify-between gap-3 p-4 rounded-xl border border-stone-200/80 bg-stone-50/50">
          <span className={`inline-flex items-center gap-1.5 px-2.5 h-7 rounded-full text-[12px] font-medium ${tone.bg} ${tone.fg}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${tone.dot}`} />{mov.type}
          </span>
          <div className={`text-[26px] font-semibold font-mono tabular-nums leading-none ${mov.qty > 0 ? "text-emerald-700" : "text-stone-800"}`}>
            {mov.qty > 0 ? "+" : ""}{mov.qty}
            <span className="text-[13px] font-normal text-stone-400 ml-1">{mov.unit}</span>
          </div>
        </div>

        {/* key-value detail */}
        <dl className="border border-stone-200/70 rounded-xl overflow-hidden divide-y divide-stone-100">
          {kv.map(row => (
            <div key={row.label} className="flex items-center justify-between gap-4 px-4 py-3">
              <dt className="text-[12px] text-stone-500">{row.label}</dt>
              <dd className={`text-[12.5px] font-medium text-right ${row.mono ? "font-mono" : ""} ${row.accent ? "text-teal-700" : "text-stone-900"}`}>{row.value}</dd>
            </div>
          ))}
        </dl>
        ...
        {mov.note && (
          <div>
            ...
            <div className="text-[12.5px] text-stone-700 leading-relaxed p-3 rounded-lg bg-stone-50 border border-stone-100">{mov.note}</div>
          </div>
        )}
      </div>
    </SlideOver>
...
function MovementsPage() {
  ...
  return (
    <div className="px-4 md:px-7 py-5 md:py-6 space-y-4">
      ...
      <Card className="p-0 overflow-hidden">
        ...
        <div className="hidden md:block">
          ...
          {filtered.map((r, i) => {
            ...
            return (
              <div key={i} onClick={() => setOpenMov(r)} className="grid grid-cols-[140px_120px_minmax(0,1fr)_110px_120px_140px] gap-3 px-5 py-3 border-b border-stone-50 last:border-0 hover:bg-stone-50/40 cursor-pointer items-center text-[12px]">
        ...
        <div className="md:hidden divide-y divide-stone-100">
          {filtered.length === 0 && (
            <div className="px-4 py-12 text-center text-[12px] text-stone-500">Nincs találat a megadott szűrőkre.</div>
          )}
          {filtered.map((r, i) => {
            const tone = movTone(r.type);
            return (
              <button key={i} onClick={() => setOpenMov(r)}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`shrink-0 inline-flex items-center gap-1 px-1.5 h-5 rounded-full text-[10px] font-medium ${tone.bg} ${tone.fg}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${tone.dot}`} />{r.type}
                    </span>
                    <span className="text-[13.5px] font-medium text-stone-900 truncate">{r.mat}</span>
                  </div>
                  <div className="text-[11px] text-stone-500 font-mono mt-0.5 truncate">{r.date} · {r.src}</div>
                </div>
                <span className={`font-mono tabular-nums text-[13px] font-semibold shrink-0 ${r.qty > 0 ? "text-emerald-700" : "text-stone-700"}`}>
                  {r.qty > 0 ? "+" : ""}{r.qty}
                  <span className="font-normal text-[10px] text-stone-400 ml-0.5">{r.unit}</span>
                </span>
      ...
      <MovementDetailSlideOver mov={openMov} onClose={() => setOpenMov(null)} />
    </div>
  );
}
...
// ──────────────────────────────────────────────────────────────────────────
function WarehouseDashboard({ onScreen }) {
  const sim = useSim();
  ...
  const trendDot = { ok: "bg-teal-500", low: "bg-amber-500", critical: "bg-rose-500" };
  const trendLabel = { ok: "Rendben", low: "Alacsony", critical: "Kritikus" };

  return (
    <div className="px-4 md:px-7 py-5 md:py-6 max-w-[1400px] mx-auto space-y-4">
      {/* KPI cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Nyilvántartott", value: stock.length, sub: "katalógus tétel", tone: "text-stone-900" },
          { label: "Riasztás",       value: alerts.length, sub: "alacsony / kritikus", tone: alerts.length ? "text-amber-700" : "text-stone-900" },
          { label: "Kritikus",       value: critical.length, sub: "azonnali beavatkozás", tone: critical.length ? "text-rose-700" : "text-stone-900" },
          { label: "Becsült érték",  value: fmtVal + " Ft", sub: "raktáron", tone: "text-stone-900" },
        ].map(c => (
          <Card key={c.label} className="p-4">
            <div className="text-[10.5px] uppercase tracking-wide text-stone-500 font-medium">{c.label}</div>
            <div className={`text-[28px] font-semibold tracking-tight mt-1 tabular-nums ${c.tone}`}>{c.value}</div>
            <div className="text-[10.5px] text-stone-500 mt-1">{c.sub}</div>
          </Card>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* Riasztások (utánrendelendő) */}
        <Card className="lg:col-span-7 p-0 overflow-hidden">
          <div className="px-4 md:px-5 py-3 border-b border-stone-200/80 flex items-center justify-between">
            <div className="text-[12.5px] font-semibold text-stone-900">Utánrendelést igényel</div>
            <button onClick={() => onScreen && onScreen("inventory")} className="text-[11.5px] text-teal-600 font-medium hover:underline">Készlet kezelése →</button>
          </div>
          {alerts.length === 0 && (
            <div className="px-5 py-10 text-center text-[12px] text-stone-400">Minden tétel a minimum szint felett van. 👍</div>
          )}
          {alerts.map(it => {
            const wh = it.worldExt.warehouse;
            const pct = Math.min(100, ((wh.onHand || 0) / (Math.max(1, (wh.min || 0) * 2))) * 100);
            return (
              <div key={it.id} className="px-4 md:px-5 py-3 border-b border-stone-100 last:border-0 flex items-center gap-3">
                <span className={`w-2 h-2 rounded-full shrink-0 ${trendDot[wh.trend]}`} />
                <div className="min-w-0 flex-1">
                  <div className="text-[12.5px] font-medium text-stone-900 truncate">{it.name}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 h-1.5 bg-stone-100 rounded-full overflow-hidden max-w-[160px]">
                      <div className={`h-full rounded-full ${wh.trend === "critical" ? "bg-rose-500" : "bg-amber-500"}`} style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-[10.5px] text-stone-400 tabular-nums">min. {wh.min}</span>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-[14px] font-semibold tabular-nums text-stone-900">{wh.onHand}<span className="text-[10px] text-stone-400 font-normal ml-0.5">{it.unit}</span></div>
                  <div className="text-[10px] font-mono text-stone-400">{wh.location || "—"}</div>
                </div>
              </div>
            );
          })}
        ...
        {/* Legutóbbi mozgások + állapot megoszlás */}
        <div className="lg:col-span-5 space-y-4">
          <Card className="p-4">
            <div className="text-[12.5px] font-semibold text-stone-900 mb-3">Készlet állapota</div>
            <div className="space-y-2">
              {[
                { k: "ok", n: ok.length },
                { k: "low", n: low.length },
                { k: "critical", n: critical.length },
              ].map(r => {
                const pct = stock.length ? (r.n / stock.length) * 100 : 0;
                return (
                  <div key={r.k} className="flex items-center gap-2.5">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${trendDot[r.k]}`} />
                    <span className="text-[11.5px] text-stone-600 w-[58px] shrink-0">{trendLabel[r.k]}</span>
                    <div className="flex-1 h-2 bg-stone-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${trendDot[r.k]}`} style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-[11.5px] tabular-nums text-stone-700 w-5 text-right">{r.n}</span>
                  </div>
                );
              })}
            </div>
          ...
          <Card className="p-0 overflow-hidden">
            <div className="px-4 py-3 border-b border-stone-200/80 flex items-center justify-between">
              <div className="text-[12.5px] font-semibold text-stone-900">Legutóbbi mozgások</div>
              <button onClick={() => onScreen && onScreen("movements")} className="text-[11.5px] text-teal-600 font-medium hover:underline">Mind →</button>
            </div>
            {recent.length === 0 && <div className="px-4 py-8 text-center text-[12px] text-stone-400">Nincs mozgás.</div>}
            {recent.map((r, i) => {
              const tone = MOV_TONE[r.type] || { bg: "bg-stone-100", fg: "text-stone-700", dot: "bg-stone-400" };
              return (
                <div key={i} className="px-4 py-2.5 border-b border-stone-50 last:border-0 flex items-center gap-2.5">
                  <span className={`shrink-0 inline-flex items-center gap-1 px-1.5 h-5 rounded-full text-[10px] font-medium ${tone.bg} ${tone.fg}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${tone.dot}`} />{r.type}
                  </span>
                  <span className="text-[12px] text-stone-800 truncate flex-1 min-w-0">{r.mat}</span>
                  <span className={`font-mono tabular-nums text-[12px] font-semibold shrink-0 ${r.qty > 0 ? "text-emerald-700" : "text-stone-700"}`}>{r.qty > 0 ? "+" : ""}{r.qty}</span>
                </div>
              );
            })}
          </Card>
