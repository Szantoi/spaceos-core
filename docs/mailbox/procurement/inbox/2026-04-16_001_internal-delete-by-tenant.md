...
# PROCUREMENT-001 — BE-TEST-06: `DELETE /internal/purchase-orders/by-tenant/{tenantId}`
...
```
DELETE /internal/purchase-orders/by-tenant/{tenantId}?confirm=true
Headers: X-SpaceOS-Internal: true
Response 200: { tenantId, deletedCounts: { purchaseOrders, deliveries } }
...
Kövesd a JOINERY-010 mintát. CLAUDE.md: WD = `/opt/spaceos/spaceos-modules-procurement/`.
Törölt entitások (sorrendben): `Deliveries` → `PurchaseOrders` (tenant szerint).
...
DONE: `mailbox/procurement/outbox/2026-04-16_001_internal-delete-by-tenant-done.md`

## Skillек & Agentек

- `/senior-backend` — Minimal API, FSM-alapú PurchaseOrder delete (csak nem-aktív PO-kat vagy az összeset?)
