...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | 915 pass / 0 fail | 0 error, 0 warning |
| **SpaceOS.Orchestrator** | L3 BFF | Node.js 22, TypeScript 5, Express 4 | `DEPLOYED` | 153 pass / 0 fail | 0 TS error |
| **SpaceOS.DesignPortal** | L4 Frontend | React 18 + Turborepo monorepo, TypeScript 5, Vite | `DEPLOYED` | 321 pass / 0 fail (275 app + 46 packages) | 0 TS error |
| **E2E** | Full stack | Vitest + fetch | `PASS` | 63 pass / 0 fail | — |

**Összesített tesztszám: 1452 pass / 0 fail**
...
| ⏳ | Migration 0020–0023 alkalmazva PostgreSQL-en | VPS deploy gate |
| ⏳ | `EXPLAIN ANALYZE` minden snapshot endpointon | PostgreSQL-en validálandó |
| ⏳ | `SELECT tableowner ... IN ('AggregateSnapshots','OutboxEntries')` → `spaceos_schema_owner` | VPS |
| ⏳ | `OutboxEntries` polling: Index Scan on `IX_OutboxEntries_Polling` | VPS |
...
- [ ] Migrations 0020–0023 alkalmazva: `dotnet ef database update`
...
- [ ] `OutboxWorker` induláskor loggol (`BackgroundService started`)
...
| ⏳ | Nginx Phase 3C: új dist path (`/apps/joinerytech/dist/`), CSP bővítés, `/brand/` location | VPS deploy gate |
| ⏳ | `pnpm install` workspace link | VPS: `npm install -g pnpm` |
| ⏳ | Migration 0024 alkalmazva PostgreSQL-en | VPS: `dotnet ef database update` |
...
- [ ] Migration 0024 alkalmazva: `dotnet ef database update`
- [ ] `npm install -g pnpm` → pnpm workspace link
- [ ] Nginx dist path frissítve: `root /opt/spaceos/design-portal/apps/joinerytech/dist/;`
- [ ] Nginx CSP bővítve (brand assets: `img-src 'self' data: blob:`)
- [ ] Nginx `/brand/` location (brand skin JSON serve)
- [ ] `brand_skin` claim megjelenik token response-ban: `curl .../bff/auth/token | jq .brandSkin`
- [ ] Doorstar skin: `joinerytech.hu`-n `JoineryTech` dashboard látszódik (default fallback)
