...
**Utolsó frissítés:** 2026-04-18 — E2E-051 teljes rerun: 128/245 pass · BUG-001/003/003b/004/005 ✅ mind live · POST /api/tenants 500 MÉG ÉL (más gyökérok) · KERNEL-090 CRITICAL kiadva · MSG-KERNEL-090 dispatched
**Tesztelő:** Gabor
...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | **1138 pass** · 7f8fd4c: MinIO DI guard ✅ · ⚠️ POST /api/tenants → 500 MÉG ÉL (KERNEL-090 vizsgálat alatt) | **0 error, 0 warning** |
...
| **SpaceOS.DesignPortal** | L4 Frontend | React 18 + Turborepo monorepo, TypeScript 5, Vite | `DEPLOYED` | **311 pass** · BUG-005 ✅ LEZÁRVA: Chat streaming end-to-end (94a79e6) · SSE URL + chunk + messagesRef mind PASS | 0 TS error · turbo build OK |
| **E2E** | Full stack | Vitest + fetch | `ACTIVE 🔴` | **128/245** — 12 fail (03/04/07: POST /api/tenants → 500 cascade) · 105 skip (tenantId undefined downstream) · BUG-001/003/003b/004/005 ✅ live · KERNEL-090 CRITICAL | — |
| **Keycloak** | IdP | Keycloak 24.0, PostgreSQL backend | `DEPLOYED` | — | hostname + Script Mapper + realm roles live |
| **SpaceOS.Modules.Joinery** | L2 Module | .NET 8, EF Core 8, PostgreSQL `spaceos_joinery` | `DEPLOYED` | **249 pass** · JOINERY-016: hardware-list-pdf + material-req-pdf · QuestPDF · commit 35a8723 · /health ✅ · ⚠️ publish chown fix szükséges (root:root → spaceos:spaceos) | 0 error, 0 warning |
...
| **SpaceOS.Modules.Inventory** | L2 Module | .NET 8 · 5004 | `DEPLOYED` | **99 pass** · BUG-003b fix LIVE: space-insensitive materialType (0ace2f7 ✅) · stock GET MDF18mm → 200 | 0 error, 0 warning |
| **SpaceOS.Modules.Cutting** | L2 Module | .NET 8 · 5005 | `DEPLOYED` | **77 pass** · BUG-004 ✅ LEZÁRVA: DateTime.SpecifyKind Utc (d8383e7) · POST /bff/cutting/plans → 201 TESTER PASS | 0 error, 0 warning |
| **SpaceOS.Modules.Procurement** | L2 Module | .NET 8 · 5006 | `DEPLOYED` | **51 pass** · BUG-001 fix LIVE: Supplier email+phone (cd5c542, migration AddSupplierPhone ✅) · TESTER reteszt folyamatban | 0 error, 0 warning |
...
**Összesített tesztszám: 2643 pass** (1138 Kernel + 219 Orchestrator + 309 Portal + 306 Doorstar Portal + 249 Joinery + 81 Abstractions + 77 Modules.Cutting · **99 Inventory** · 51 Procurement · 41 Modules.Contracts + 21 Reservation Contracts · E2E: **128/245** 🔴 KERNEL-090 blokkoló)

**Tudásbázis:** `docs/knowledge/` — 18 fájl · LIB-001 DONE · security / deployment / patterns / architecture / context · Librarian terminál: `spaceos-librarian/CLAUDE.md`
...
| **Bug Fix Sprint — Soft Launch 1** | BUG-001/002/003/003b/004/005 mind LEZÁRVA ✅ · E2E-051 rerun: 128/245 (12 fail, 105 skip) · POST /api/tenants 500 MÉG ÉL → KERNEL-090 vizsgálat | `ACTIVE 🔴` | KERNEL-090: POST /api/tenants → 500 (gyökérok: KERNEL-086 audit chain vagy RLS) | 2026-04-18 |
