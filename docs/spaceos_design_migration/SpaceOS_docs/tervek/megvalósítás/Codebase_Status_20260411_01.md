...
**Utolsó frissítés:** 2026-04-11 (Orchestrator Stage Dispatch DONE — 176 teszt · Kernel Stage Registry kód elfogadva, tesztek hiányoznak MSG-KERNEL-057 · Infra Keycloak hostname fix MSG-INFRA-056 folyamatban)
**Tesztelő:** Gabor
...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | 933 pass / 0 fail | 0 error, 0 warning |
| **SpaceOS.Orchestrator** | L3 BFF | Node.js 22, TypeScript 5, Express 4 | `DEPLOYED` | 176 pass / 0 fail | 0 TS error |
| **SpaceOS.DesignPortal** | L4 Frontend | React 18 + Turborepo monorepo, TypeScript 5, Vite | `DEPLOYED` | 291 pass / 0 fail | 0 TS error |
| **E2E** | Full stack | Vitest + fetch | `UPDATED` | 109 pass / 0 fail | Keycloak DAG migráció DONE |
| **Keycloak** | IdP | Keycloak 24.0, PostgreSQL backend | `PENDING_INFRA` | — | MSG-INFRA-KC01 kiadva |
| **SpaceOS.Modules.Joinery** | L2 Module | .NET 8, EF Core 8, PostgreSQL `spaceos_joinery` | `DEPLOYED` | 109 pass / 0 fail | 0 error, 0 warning |
| **SpaceOS.Modules.Abstractions** | L2 Module | .NET 8, EF Core 8, PostgreSQL `spaceos_modules` | `ACTIVE` | 61 pass / 0 fail | 0 error, 0 warning |

**Összesített tesztszám: 1570+ pass / 0 fail** (933 Kernel + 176 Orchestrator + 291 Portal + 109 Joinery + 61 Abstractions + 109 E2E - 109 overlap)
...
| Keycloak IdP v4 — Kernel (MSG-KC01) | JWT JWKS + TenantSessionInterceptor + JwksHealthCheck | `CODE_REVIEW` | Infra Track A | T1-T5 DONE — 933 teszt ✅ |
| Keycloak IdP v4 — Orchestrator (MSG-KC02) | jwks-rsa + /bff/auth/me + régi auth törlés | `CODE_REVIEW` | — | T1-T4 DONE — 176 teszt ✅ |
| Keycloak IdP v4 — Portal (MSG-KC03) | PKCE state+nonce + AuthStore + CallbackPage | `CODE_REVIEW` | — | T1-T6 DONE — 291 teszt ✅ |
| Keycloak IdP v4 — E2E migráció | helpers.ts + global-setup + 28-keycloak-auth + refresh/auth chain tesztek | `DONE` | Infra deploy | Keycloak DAG, 28-keycloak-auth.chain.test.ts (14 teszt) |
| Keycloak IdP v4 — Infra (MSG-INFRA-KC01) | VPS: spaceos_keycloak DB + realm + clients + groups + nginx | `PENDING` | VPS Operator | Blokkol: E2E live run |
| Keycloak hostname fix (MSG-INFRA-056) | keycloak.conf hostname=joinerytech.hu → JWT iss mismatch javítás | `ACTIVE` | VPS Operator | Blokkol: E2E JWT 401 hibák |
| **Kernel Stage Registry** | StageDefinition + StageChain + StageHandoff + Migration 0028 + 15 endpoint | `CODE_COMPLETE` | tesztek hiányoznak (MSG-057) | 933 teszt ✅ · kód elfogadva |
| Modules.Joinery v2 | PDF export, Gyártásilap | `BACKLOG` | Stage Registry DoD | — |
...
**Implementáció:** MSG-KERNEL-054-DONE elfogadva 2026-04-10 — kód rendben
**Tesztek:** MSG-KERNEL-057 (visszadobva) — ≥45 teszt hiányzik (0 db meglévő 933-ból új)
...
- [ ] **Stage Registry új tesztek: ≥45 db** ← **HIÁNYZIK (MSG-KERNEL-057)**
- [x] 0 build warning ✅
...
| **B — Kernel** | spaceos-kernel | JWT JWKS Authority + TenantSessionInterceptor double-deser (BE-01) + active tenant validation (DB-02) + JwksHealthCheck (BE-02) + régi auth törlés | ✅ `CODE_REVIEW` — 933 teszt | MSG-KC01-RESP |
| **C — Orchestrator** | spaceos-orchestrator | `jwks-rsa` + JWKS verify + retry on key rotation (BE-03) + `GET /bff/api/auth/me` + régi auth routes törlés | ✅ `CODE_REVIEW` — 163 teszt | MSG-KC02-RESP |
| **D — Portal** | spaceos-design-portal | PKCE state+nonce (SEC-01/02) + `CallbackPage` + `AuthStore` rewrite (memory-only) + `ProtectedRoute` + régi login törlés | ✅ `CODE_REVIEW` — 291 teszt | MSG-KC03-DONE |
...
| — | Keycloak IdP v4 — OIDC PKCE + JWKS (MSG-KC01/02/03) | P0 | `PLANNED` — feladatok kiosztva 2026-04-09 |
