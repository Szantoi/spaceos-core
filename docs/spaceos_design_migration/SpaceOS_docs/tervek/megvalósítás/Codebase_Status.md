...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | 686 pass / 0 fail | 0 error, 0 warning |
| **SpaceOS.Orchestrator** | L3 BFF | Node.js 22, TypeScript 5, Express 4 | `DEPLOYED` | 76 pass / 0 fail | 0 TS error |
| **SpaceOS.DesignPortal** | L4 Frontend | React 18, TypeScript 5, Vite, Tailwind | `DEPLOYED` | 224 pass / 0 fail | 0 TS error |
| **E2E** | Full stack | Vitest + fetch | `PASS` | 63 pass / 0 fail | — |

**Összesített tesztszám: 1049 pass / 0 fail**
...
## Teszteredmények részletezés (2026-04-06)

### Kernel — 686 pass / 0 fail

| Teszt projekt | Pass | Fail |
|---|---|---|
| SpaceOS.Kernel.Tests (unit) | 541 | 0 |
| SpaceOS.Kernel.Api.Tests (API) | 53 | 0 |
| SpaceOS.Kernel.IntegrationTests | 92 | 0 |
...
| chat.route.test.ts | 5 |
| health.route.test.ts | 3 |
| auth.route.test.ts | 15 |
| interpreter.service.test.ts | 12 |
...
### Design Portal — 224 pass / 0 fail (30 fájl)
...
## Nyitott fejlesztések (Sprint D Phase 2+)

| ID | Feladat | Prioritás | Státusz |
|---|---|---|---|
| D-P2 | Tool Registry — valós Kernel integráció (chat → FlowEpic lekérdezés) | P1 | `NYITOTT` |
| D-P3 | Modules.Joinery MVP (ajtóméretek, anyaglista, vágási terv) | P1 | `NYITOTT` |
| D-P4 | Multi-brand architektúra (Turborepo, brand skin-ek) | P2 | `NYITOTT` |
| — | Dashboard statisztika kártyák (E27 befejezése) | P3 | `NYITOTT` |
| — | SSE streaming a /bff/chat endpoint-on | P3 | `NYITOTT` |
...
| SSE streaming | `/bff/chat` full JSON válasz, streaming nincs implementálva |
