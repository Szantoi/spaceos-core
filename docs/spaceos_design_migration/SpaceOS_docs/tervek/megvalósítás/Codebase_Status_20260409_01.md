...
**Utolsó frissítés:** 2026-04-09 (Modules.Joinery v1 CLOSED_DONE + Doorstar Onboarding CLOSED_DONE — 78 teszt, 15 DoorTypeRule, 41 ProcessTask; következő: Modules.Abstractions v1)
**Tesztelő:** Gabor
...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | 927 pass / 0 fail | 0 error, 0 warning |
| **SpaceOS.Orchestrator** | L3 BFF | Node.js 22, TypeScript 5, Express 4 | `DEPLOYED` | 176 pass / 0 fail | 0 TS error |
| **SpaceOS.DesignPortal** | L4 Frontend | React 18 + Turborepo monorepo, TypeScript 5, Vite | `DEPLOYED` | 346 pass / 0 fail | 0 TS error |
| **E2E** | Full stack | Vitest + fetch | `PASS` | 109 pass / 0 fail | — |
| **Keycloak** | IdP | Keycloak 24.0, PostgreSQL backend | `RUNNING` | — | spaceos realm + 3 client |
| **SpaceOS.Modules.Joinery** | L2 Module | .NET 8, EF Core 8, PostgreSQL `spaceos_joinery` | `DEPLOYED` | 78 pass / 0 fail | 0 error, 0 warning |

**Összesített tesztszám: 1527+ pass / 0 fail** (927 Kernel + 176 Orchestrator + 346 Portal + 78 Joinery + 109 E2E - 109 overlap)
...
| Modules.Abstractions v1 | Product Configuration Engine: Parametric Component Graph + Manufacturing Derivation + IFC/STEP Bridge | `PLANNED` | Phase 3C+ DoD ✅ | 46 nap |
| Modules.Joinery v2 | PDF export, Gyártásilap | `BACKLOG` | Joinery v1 DoD | — |
...
| DB migrációk alkalmazása | ⏳ Kiadva (MSG-J042) | Program.cs MigrateAsync + spaceos_joinery schema |
| Seed bővítés | ⏳ Kiadva (MSG-J042) | DoorTypeRules 8→15+, ProcessTaskTemplates 10→40+ |
| Tesztek bővítése | ⏳ Kiadva (MSG-J042) | 32→≥65: API integrációs + security + handler tesztek |
...
- [x] 78 Modules.Joinery teszt zöld (≥65 DoD teljesítve)
...
- [x] `DoorstarSeedData.cs` compile OK + seed validation tesztek zöldek ✅ 78 teszt
...
## Modules.Abstractions v1 — Product Configuration Engine (PLANNED)

**Ref:** `/opt/spaceos/docs/SpaceOS_Modules_Abstractions_Architecture_v4.md`
**Repo:** `spaceos-modules-abstractions` (ÚJ polyrepo)
**DB schema:** `spaceos_modules` (meglévő PostgreSQL 16)
**Becsült effort:** ~46 fejlesztői nap (4 track)
**Blokkoló feltétel:** Phase 3C+ DoD ✅ (Kernel migrations 0025-0026 kész)
**Review összesítő:** 3 CRITICAL · 8 HIGH · 7 MEDIUM finding (v4-ben mind beépítve)

### Scope (4 track)

| Track | Effort | Tartalom |
|-------|--------|---------|
| **A-Core** | ~15 nap | ProductTemplate aggregate + ComponentSlot + SlotConnection + GraphCalculationEngine (Kahn's BFS) |
| **B-Manufacturing** | ~8 nap | CNC deriválás (JointType → MachiningOp) + ProcessPhase → ProductionStep + CuttingListItem |
| **C-Geometry** | ~12 nap | GeometryAttachment (L0-L4) + BVH bridge (Phase 3A SpatialElement) |
| **D-IFC** | ~11 nap | IFC/STEP parser (Horizon 2, opcionális) |

### Kritikus finding-ek

| ID | Súly | Probléma | Javítás |
|----|------|----------|---------|
| DB-01 | 🔴 CRITICAL | DAG cycle: SlotConnection kört tartalmazhat → végtelen rekurzió | `check_connection_dag()` PostgreSQL trigger (recursive CTE cycle detection) |
| SEC-01 | 🔴 CRITICAL | Cross-tenant template access — RLS hiánya | RLS FORCE mind az 5 táblán |
| SEC-02 | 🔴 CRITICAL | GeometryAttachment FileReference path traversal | Regex validáció + storage prefix enforced |
| BE-02 | 🟠 HIGH | TopologicalSort recursive → stack overflow mély gráfon | Iteratív Kahn's algorithm (BFS) |

### Definition of Done (kapuk)

- [ ] Migration 0001 alkalmazva (spaceos_modules schema, 5 tábla, RLS, DAG trigger)
- [ ] `check_connection_dag()` trigger: kör detektálva → rollback
- [ ] GraphCalculationEngine: Kahn's topological sort, iteratív
- [ ] `Math.Round(_, 1, MidpointRounding.AwayFromZero)` minden számításban
- [ ] Cross-tenant template → RLS blocked
- [ ] FileReference path traversal → blocked
- [ ] Meglévő tesztek zöld + ≥60 új Modules.Abstractions teszt
