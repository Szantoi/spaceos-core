...
**Utolsó frissítés:** 2026-04-09 (Modules.Abstractions v1 Phase B-Manufacturing DONE — 61 teszt; Keycloak IdP v4 feladatok kiosztva: KC01/KC02/KC03)
**Tesztelő:** Gabor
...
**Összesített tesztszám: 1619+ pass / 0 fail** (927 Kernel + 176 Orchestrator + 346 Portal + 109 Joinery + 61 Abstractions + 109 E2E - 109 overlap)
...
| Keycloak IdP v4 — Kernel (MSG-KC01) | JWT JWKS + TenantSessionInterceptor + JwksHealthCheck | `PLANNED` | Infra Track A | ~4 nap |
| Keycloak IdP v4 — Orchestrator (MSG-KC02) | jwks-rsa + /bff/auth/me + régi auth törlés | `PLANNED` | Kernel kész | ~2 nap |
| Keycloak IdP v4 — Portal (MSG-KC03) | PKCE state+nonce + AuthStore + CallbackPage | `PLANNED` | Orchestrator kész | ~3 nap |
...
| **B — Kernel** | spaceos-kernel | JWT JWKS Authority + TenantSessionInterceptor double-deser (BE-01) + active tenant validation (DB-02) + JwksHealthCheck (BE-02) + régi auth törlés | `PLANNED` | MSG-KC01 |
| **C — Orchestrator** | spaceos-orchestrator | `jwks-rsa` + JWKS verify + retry on key rotation (BE-03) + `GET /bff/api/auth/me` + régi auth routes törlés | `PLANNED` | MSG-KC02 |
| **D — Portal** | spaceos-design-portal | PKCE state+nonce (SEC-01/02) + `CallbackPage` + `AuthStore` rewrite (memory-only) + `ProtectedRoute` + régi login törlés | `PLANNED` | MSG-KC03 |
...
- [ ] Kernel: JWKS auto-discovery, `OnTokenValidated` realm roles, `TenantSessionInterceptor` Keycloak claims
- [ ] Kernel: régi `LoginCommand` + `TokenService` + `/api/auth/*` ELTÁVOLÍTVA
...
- [ ] ≥53 új teszt · meglévő 1452 teszt ZÖLD · 0 build warning
- [ ] `grep -r "auth/token\|generateToken\|loginCommand" → 0`
