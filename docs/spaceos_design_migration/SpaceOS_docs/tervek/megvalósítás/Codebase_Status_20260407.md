...
|---|---|---|---|---|---|
| **SpaceOS.Kernel** | L2 Backend | .NET 8, EF Core 8, PostgreSQL | `DEPLOYED` | 777 pass / 0 fail | 0 error, 0 warning |
...
**Összesített tesztszám: 1210 pass / 0 fail**
...
## Sprint D Phase 3A — Spatial BIM Core (2026-04-07) — FOLYAMATBAN

**Ref:** `/opt/spaceos/docs/SpaceOS_Phase3A_Architecture_v3.md`
**Mailbox:** MSG-K024 (UNREAD — implementáció folyamatban)
**Becsült effort:** ~10 fejlesztői nap

| Terület | Tartalom | Státusz |
|---|---|---|
| PA-01 | Domain Layer (PhysicalSpace, BvhNode, SpatialElement, SpatialTaskLink, 5 VO, 3 event) | `FOLYAMATBAN` |
| PA-02 | BVH Domain Service (IBvhTreeService, BvhQueryService, cycle+depth guard) | `FOLYAMATBAN` |
| PA-03 | Application CQRS (5 handler + 3 FluentValidation validator) | `FOLYAMATBAN` |
| PA-04 | Infrastructure (EF Core OwnsOne, IBvhRepository, Migrations 0016–0019) | `FOLYAMATBAN` |
| PA-05 | API (5 publikus endpoint; BVH belső — nem OpenAPI) | `FOLYAMATBAN` |
| PA-06 | Tesztek (≥35 új: domain, security, integration gate-ek) | `FOLYAMATBAN` |
...
| **Sprint D P3A** | **Spatial BIM Core + Modules.Joinery + 4D Timeline** | **`FOLYAMATBAN`** |
...
### Kernel — 777 pass / 0 fail (+91 Sprint D Phase 2)

| Teszt projekt | Pass | Fail |
|---|---|---|
| SpaceOS.Kernel.Tests (unit) | 608 | 0 |
| SpaceOS.Kernel.Api.Tests (API) | 101 | 0 |
| SpaceOS.Kernel.IntegrationTests | 68 | 0 |
...
|---|---|---|---|
| D-P3A | Spatial BIM Core + Modules.Joinery + 4D Timeline (Phase 3A) | P0 | `FOLYAMATBAN` (MSG-K024) |
